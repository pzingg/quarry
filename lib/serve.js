      ...
      pg.connect(database.connectionString, function (error, client, done) {
        ...
        request.on('end', function () {
          ...
          switch (action) {
            ...
            case 'findAll':
              q = q_.getQuery(request.params, { max_results: maxResults });
              if (maxResults > 0) {
                query = 'SELECT COUNT(*) AS count FROM ' + tableName;
                if (q['WHERE']) { 
                  query += q['WHERE'];
                  queryParams = q['WPARAMS'];
                }
              ...
              if (q['ORDER']) query += q['ORDER'];
              if (q['LIMIT']) query += q['LIMIT'];
              if (q['OFFSET']) query += q['OFFSET'];
              util.logNotice('findAll query $1 with params $2', query, queryParams);
              break;
